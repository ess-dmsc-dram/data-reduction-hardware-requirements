...
class LatexTabular:
    ...
    def add(self, power, use_fraction, event_rate, t_run, t_reduction, *args):
        ...
        if t_reduction > 30 and t_run < self.speedup*t_reduction:
            base = ('& {:4.1f} & {:2.1f} & ${}\cdot 10^{:1}$ & {:8.1f} & \\textcolor{{red}}{{{:6.0f}}} & {:4} & {:6.0f} & {:4.0f}' + '\\\\').format(power, 100*use_fraction, event_rate_base, event_rate_exponent, t_run, t_reduction, *args)
        else:
            base = ('& {:4.1f} & {:2.1f} & ${}\cdot 10^{:1}$ & {:8.1f} & {:6.0f} & {:4} & {:6.0f} & {:4.0f}' + '\\\\').format(power, 100*use_fraction, event_rate_base, event_rate_exponent, t_run, t_reduction, *args)
        if self.power == 0:
