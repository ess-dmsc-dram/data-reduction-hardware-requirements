...
\begin{table}
  ...
  \begin{tabular}{lrrl}
    ...
    \bifrost & 50k & 50k & 2--4 scan positions\\
    \cspec & 400k & 750k \\
    % 1.25*32*6.2/(0.003*0.006) = 14000k
    \dream & 762k & 2167k & maybe more?\footnote{This appears to be a moving target and no one really knows. Current ``full coverage'' is $5.11~\mathrm{sr}$ with a sample-detector distance of $1.25~\mathrm{m}$. Day-1 coverage is $1.81~\mathrm{sr}$.}\\
    \estia & 250k & 500k \\
    \loki & 750k & 1500k \\
...
14 out of the 21 cases given in these figures can be handled even with high accelerator power of more than 3 MW.
...
However, we will definitely require MPI support in live reduction for \emph{most} configurations of \dream from day one.
...
It is however not unlikely that we will require at least some of it in \mantid, e.g., for a visualization during the experiment.
A few specialized machines with of the order of $1~\mathrm{TByte}$ RAM or are likely to be required.
...
The requirement can potentially be lowered further by reducing $\Nworkspace$ and/or by avoiding binning of most workspaces, i.e., working purely in event mode.
}
\end{figure}
...
It should also be noted that while our benchmarks include the event-mode powder-diffraction workflow which will also be used for \dream, the benchmarks are based on POWGEN with only 43121 pixels, i.e., we are dealing with an extrapolation over nearly two orders of magnitude.
}
\end{table}
