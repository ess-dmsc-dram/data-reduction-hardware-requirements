...
The assumption that a histogram-representation for all workspaces is required leads to a rather high baseline requirement --- scaling with the total event count only becomes visible near $10^9$ events and above.
...
`Use' is (usually) a guess of the fraction a particular instrument mode/configuration is used.
...
The biggest uncertainty comes from the duration of a single run.
...
If a combined treatment of the sample scan is possible the requirement is likely to be much lower in the high-flux case.
...
Not considering 
few events per pixel, is our estimate good? overhead from eventlist?
TODO get better estimate on run duration before writing interpretiona. may get worse if there are fewer events per run than currently estimated
}
\end{table}
...
Working in event mode we have to following options:
\begin{enumerate}
  \item For visualization purposes, on-the-fly binning of event data could be implemented.
  \item The transition to histogram mode can be done histogram-by-histogram at the point of histogram summation, e.g., for powder diffraction workflows in the algorithm \verb|DiffractionFoccusing| used for \dream.
...
TODO
discuss strong influence of run duration (and the ratio of events to pixel count where either contribution becomes dominant)
...
It is worth noting that a large part of the huge memory requirements for \dream, \magic, and also \cspec come from the large number of detector pixels resulting in a large $\Nspec$.
...
Once could argue that we can thus project the voxel data onto a grid of pixels, gaining an order of magnitude reduction of $\Nspec$.
However, cannot rely on that at this point since there are a couple of complications:
\begin{enumerate}
...
- list of things that need to be clarified?
- can we get away without using histograms, e.g., for \dream, running diffraction focussing on an event workspace, rebinning only the spectrum that is currently being processed (if they want to llok at the raw data, rebin interactively?!)
