...
infile = "bench_log_003.txt";
outfile = "SNSPowderReduction_bins_bench.txt"

fin = open(infile, 'r')
fout = open(outfile, 'w')

# Define data columns
header = ["Run no.","NCPU","Runtime","Nbins"]
ncols = len(header)
...
# Go through log file content
for line in fin:
    if line.count("NBINS : PG3_77777") > 0:
        NBins = float(line.split(' ')[-1].rstrip())
    ...
    if line.count("NCPU") > 0:
        ...
        data[run_count][1] = ncpu
        data[run_count][2] = time_cpu
        
        data[run_count][3] = NBins
