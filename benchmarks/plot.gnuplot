...
p \
  'PG3-event-mode-scale-0.1' u 1:2 w lp ti "default binning" ls 100, \
...
p \
  "< cat data/PG3-event-mode-scale-0.1 | grep '\^1 ' | cut -d' ' -f2- | tr ' ' '\n'" u (2**$0/4):1 ti "1 rank", \
  "< cat data/PG3-event-mode-scale-0.1 | grep '\^2 ' | cut -d' ' -f2- | tr ' ' '\n'" u (2**$0/4):1 ti "2 ranks", \
  "< cat data/PG3-event-mode-scale-0.1 | grep '\^4 ' | cut -d' ' -f2- | tr ' ' '\n'" u (2**$0/4):1 ti "4 ranks", \
  "< cat data/PG3-event-mode-scale-0.1 | grep '\^8 ' | cut -d' ' -f2- | tr ' ' '\n'" u (2**$0/4):1 ti "8 ranks"
